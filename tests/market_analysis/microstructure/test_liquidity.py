...
def sample_trade_data():
    ...
    # Generate volume with known relationships to price changes
    base_volume = np.random.lognormal(8, 1, 1000)
    price_impact = 0.1  # Known Kyle's lambda
    
    # Adjust prices based on volume (for known price impact)
    price_changes = np.diff(price)
    price[1:] += price_impact * base_volume[1:] / 10000
...
def test_kyle_lambda_calculation(liquidity_analyzer):
    ...
    assert (lambda_series.dropna() > 0).all()
    ...
    assert lambda_5d.std() < lambda_1d.std()  # Longer window should be more stable
...
def test_amihud_ratio(liquidity_analyzer):
    ...
    assert (ratio >= 0).all()  # Non-negative by definition
    
    # Test scaling
    ratio1 = liquidity_analyzer.calculate_amihud_ratio(scaling=1e6)
    ratio2 = liquidity_analyzer.calculate_amihud_ratio(scaling=1e3)
    np.testing.assert_almost_equal(ratio1 * 1e-3, ratio2)


def test_bid_ask_bounce(liquidity_analyzer):
    ...
    assert (bounce.dropna() >= 0).all()
    assert (bounce.dropna() <= 0.01).all()  # 100bps max
    
    # Test without quote data
    analyzer_no_quotes = LiquidityAnalyzer(
        trades=sample_trade_data(),
        quotes=None
    )
...
def test_data_validation():
    ...
    bad_data = pd.DataFrame({
        ...
        'price': np.random.randn(10)  # Missing volume
    })
