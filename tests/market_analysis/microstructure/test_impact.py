...
def sample_trade_data():
    ...
    data = pd.DataFrame({
        'timestamp': dates,
        'price': 100 * (1 + np.random.randn(1000) * 0.001),
        'volume': np.random.lognormal(4, 1, 1000),  # Using 'volume' consistently
...
def test_illiquid_market_conditions(illiquid_market_data):  # Added parameter
    """Test impact models under illiquid market conditions."""
    analyzer = MarketImpact(trades=illiquid_market_data)
